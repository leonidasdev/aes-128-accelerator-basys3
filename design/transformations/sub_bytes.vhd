...
architecture rtl of sub_bytes is
    ...
    -- FIPS-197 S-Box (forward substitution table) - verified canonical (all 256 entries)
    ...
    -- Convert 128-bit state to 16 bytes and apply S-Box
    function apply_sbox(state : std_logic_vector(127 downto 0)) return std_logic_vector is
        variable result : std_logic_vector(127 downto 0);
        variable byte_in : std_logic_vector(7 downto 0);
    begin
        for i in 0 to 15 loop
            byte_in := state(i * 8 + 7 downto i * 8);
            result(i * 8 + 7 downto i * 8) := sbox(to_integer(unsigned(byte_in)));
        end loop;
        return result;
    end function apply_sbox;

begin

    state_out <= apply_sbox(state_in);
