--
-- Purpose:   AES InvSubBytes transformation (inverse S-Box substitution).
-- Author:    PHR26-T03
-- Date:      29/04/2026
--
-- Description:
-- This module performs the InvSubBytes transformation of AES decryption.
-- It substitutes each byte using the inverse S-Box from FIPS-197.
...
architecture rtl of inv_sub_bytes is
    ...
    function apply_inv_sbox(state : std_logic_vector(127 downto 0)) return std_logic_vector is
        variable result : std_logic_vector(127 downto 0);
        variable byte_in : std_logic_vector(7 downto 0);
    begin
        for i in 0 to 15 loop
            byte_in := state(i * 8 + 7 downto i * 8);
            result(i * 8 + 7 downto i * 8) := inv_sbox(to_integer(unsigned(byte_in)));
        end loop;
        return result;
    end function apply_inv_sbox;

begin

    state_out <= apply_inv_sbox(state_in);
